...
$(document).ready(function () {
    ...
    Papa.parse("chips.csv", {
        ...
        complete: function (results) {
            ...
            const tbody = $('#chipsTable tbody');
            const brandSet = new Set();
            const tasteSet = new Set();

            data.forEach(row => {
                const imgTag = `<img src="${row.Pic}" alt="${row.Taste}" width="50">`;
                const tableRow = `
                    <tr>
                        <td>${row.Brand}</td>
                        <td>${row.Taste}</td>
            ...
            const tasteFilter = $('#tasteFilter');
            Array.from(tasteSet).sort().forEach(taste => {
                tasteFilter.append(`<option value="${taste}">${taste}</option>`);
            });

            // Initialize DataTables
            const table = $('#chipsTable').DataTable();

            // Custom filtering for Brand
            $('#brandFilter').on('change', function () {
                const brand = $(this).val();
                table.column(1).search(brand).draw(); // Column index 1 is "Brand"
            });

            // Custom filtering for Taste
            $('#tasteFilter').on('change', function () {
                const taste = $(this).val();
                table.column(2).search(taste).draw(); // Column index 2 is "Taste"
            });
        }
    });
});
